...
# (opcional) remover linhas sem data antes de registrar:
# df_pedidos = df_pedidos.dropna(subset=["data_pedido_ts"]).reset_index(drop=True)

# cria/define a conexão DuckDB antes de registrar!
conn = duckdb.connect(database=":memory:")
...
WITH parsed AS (
  SELECT
    DATE_TRUNC('day', STRPTIME(p.data_pedido_norm, '%Y-%m-%d %H:%M:%S'))::DATE AS dia,
    c.grupo AS grupo,
    COUNT(*) AS cnt
  FROM pedidos p
  JOIN clientes c ON p.cliente_id = c.cliente_id
